...
way to visually inspect the data.  The data were also separated visually between pitchers and position players to see if there is much of a difference.

```{r, message=F}
war %>% mutate(Pos = ifelse(position=="Pitcher","Pitcher","Position Player")) %>% ggplot(aes(x=values,fill=factor(Pos))) + geom_dotplot() + labs(x="W.A.R.", y = "Count")
```
